...
Internara is structured with a clear separation between **Global Application Concerns** (framework & infrastructure) and **Modular Domain Concerns**.

### 1.1 Global Application Layer

This layer encompasses elements fundamental to the entire Laravel application's operation and are *not* directly tied to a specific business module. These include:
*   **Framework Configuration:** Global configuration files (e.g., `config/app.php`, `config/database.php`).
*   **Environment Settings:** `.env` file and related environment-specific configurations.
*   **Core Application Providers:** Global service providers (e.g., `app/Providers/AppServiceProvider.php`, `app/Providers/BindServiceProvider.php`) responsible for application-wide services, infrastructure integrations, and global fallbacks.
*   **Framework Integrations:** Top-level integrations that affect how the Laravel framework operates across all modules.

These elements reside in the application's root directory, `config/` directory, or `app/` directory.

### 1.2 Modular Domain Layer

This layer embodies the **Modular Monolith** pattern, where Internara is a collection of self-contained mini-applications (called **Modules**) residing within the `modules/` directory. Each module (e.g., `User`, `Internship`) manages a specific business domain, enhancing organization, maintainability, and scalability.

**Key Concepts for Modular Layer:**

*   **Modular Monolith:** Leveraging `nwidart/laravel-modules` to encapsulate specific business domains comprehensively.
*   **Isolation & Portability:** Each module is designed to be as self-contained as possible for its domain.
    *   **Independence:** Modules **MUST NOT** depend on the physical existence or internal logic of other modules directly.
    ...
    *   **Self-Containment:** A module encapsulates its UI, business logic, and data structures. It should be extractable with minimal friction.
*   **UI Framework:** All user interfaces and interactivity within modules are built using **Livewire** components, powered by the TALL Stack.
*   **Module-Specific Backend Structure:** Logic within each module is organized into distinct layers:
    ...
    *   **Models:** Represent the module's persisted domain data and contain limited, domain-relevant behavior.
    *   **Optional Layers (Repositories/Entities):** Abstractions for complex data access or integrations, **not included by default within modules**.
...
For a complete, step-by-step example of creating a new feature within this architecture, refer to the **[Modular Monolith Developer Guide](modular-monolith-workflow.md)**.
