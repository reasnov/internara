...
**Important Note on Naming & Namespaces:** When using `module:make-*` commands, always refer to the [Development Conventions](development-conventions.md) for guidelines on naming and namespace structures, particularly regarding the omission of the `app` segment in namespaces.
