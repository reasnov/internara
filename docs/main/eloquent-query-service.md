...
The `EloquentQuery` base service is a core architectural component of Internara. it provides a standardized, boilerplate-free implementation for services that primarily perform database operations on a single Eloquent model.

## Purpose

-   **Standardization:** Ensures all domain services have a consistent API for CRUD and querying.
-   **Efficiency:** Reduces repetitive code by providing common logic for pagination, filtering, searching, and caching.
-   **Security:** Enforces mass-assignment protection and safe querying patterns.
-   **Abstraction:** Decouples the UI and Business Logic layers from direct Eloquent builder manipulation.
...
You can set a base query (e.g., with global scopes or eager loading) that all subsequent queries will build upon:
```php
