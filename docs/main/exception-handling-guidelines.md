...
*   **Purpose:** To encapsulate business logic violations or other application-specific issues, offering a consistent and translatable way to communicate these problems. This exception class is part of the `Exception` module.
*   **Key Features:**
...
1.  **Shared Exception Translations**: For messages intended to be reused across multiple modules. These are further categorized by their scope:
    *   **Truly Global Errors**: For system-wide issues not tied to any specific domain (e.g., service unavailable, internal server error). If defined, these would reside in the `Exception` module's exceptions file.
