...
abstract class RecordManager extends Component
{
    ...
    {
        try {
            $collection = collect($this->records->items());

            // Enrich each item with computed actions based on RBAC
            $this->items = $collection
                ->map(function ($item) {
                    $itemArray = $item->toArray();
                    $itemArray['actions'] = $this->getActionsForRecord($item);
    ...
    /**
     * Computed row actions for table rows.
    ...
    protected function getActionConfig(string $action, mixed $record = null): array
    {
        return match ($action) {
            'edit' => [
                'icon' => 'tabler.edit',
                'variant' => 'tertiary',
                'class' => 'text-info btn-circle btn-xs hover:bg-info/10',
                'wire' => "edit('{$record?->id}')",
            ...
            'delete' => [
                'icon' => 'tabler.trash',
                'variant' => 'tertiary',
                'class' => 'text-error btn-circle btn-xs hover:bg-error/10',
                'wire' => "discard('{$record?->id}')",
    ...
    public function getActionsForRecord(mixed $record): array
    {
        $actions = [];
        $availableActions = $this->rowActions();

        foreach ($availableActions as $action) {
            $config = $this->getActionConfig($action, $record);
            $can = $config['can'] ?? $action;
            if ($this->can($can, $record)) {
                $actions[] = [
                    'key' => $action,
                    'icon' => $config['icon'],
