...
<div x-data="{
        ...
        addFiles(newFiles) {
            ...
            if (this.isMultiple) {
                ...
                this.files.filter(f => f.isNew).forEach(f => URL.revokeObjectURL(f.url));
        ...
        updateFileInput() {
            ...
            this.files.filter(f => f.isNew).forEach(f => {
    ...
    <div class="mt-2 h-full min-h-24 w-full rounded-lg border border-gray-300 p-4 dark:border-gray-300">
        <div class="border-3 flex h-full w-full cursor-pointer items-center justify-center rounded-lg border-dashed border-gray-300 p-4 hover:border-gray-500 dark:border-gray-700"
            ...
            <div class="text-center text-xs text-gray-500 relative w-full" x-on:click.prevent="!isDropping && $refs.input.click()">
                                ...
                                <template x-for="fileItem in files" :key="fileItem.id">
                                    <li class="flex items-center gap-2 p-2">
                                        ...
                                        <template x-if="isImage(fileItem) && fileItem.url">
                                            <img x-bind:src="fileItem.url" class="h-8 w-8 flex-shrink-0 rounded-md object-cover" alt="Thumbnail">
                                        </template>
                                        <template x-if="!isImage(fileItem) || !fileItem.url">
                                        ...
                                        {{-- Remove Button --}}
                                        <button type="button" title="Remove file" x-on:click.stop="removeFile(fileItem.id)" class="btn btn-ghost btn-circle btn-xs flex-shrink-0">
                            ...
                            <div class="flex items-center justify-center">
                                ...
                                <div class="relative">
                                    ...
                                    <template x-if="isImage(files[0])">
                                        <img x-bind:src="files[0].url" alt="File Preview" class="h-32 w-32 rounded-lg object-cover">
                                    </template>
                                    <template x-if="!isImage(files[0])">
                                    ...
                                    {{-- Remove Button --}}
                                    <button type="button" title="Remove file" x-on:click.stop="removeFile(files[0].id)" class="btn btn-primary btn-circle btn-xs absolute -right-2 -top-2 z-10">
                                        <x-ui::icon name="tabler-x" class="h-4 w-4" />
                                    </button>
                                </div>
                            </div>
