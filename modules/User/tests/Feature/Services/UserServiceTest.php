...
test(
    'UserService assigns student role by default if not specified and sends welcome notification',
    function () {
        Notification::fake();
        $userService = app(UserService::class);
...
test('SuperAdmin cannot be deleted via UserService', function () {
    ...
    expect(fn () => $userService->delete($superAdmin->id))->toThrow(AuthorizationException::class);
});
